...
#define CHECK_COND(_cond) \
    ...
    do
    {
        ...
        {
            pindex->SA = (saidx_t *) malloc((pindex->n / CSA_D2 + 1)
                                            * sizeof(saidx_t));
            if (!pindex->SA)
            {
        ...
        {
            fseeko(fp, (pindex->n / CSA_D + 1) * CSA_K, SEEK_CUR);
        }
        ...
        {
            pindex->ISA = (saidx_t *) malloc(((pindex->n - 1) / CSA_D2 + 1)
                                             * sizeof(saidx_t));
            if (!pindex->ISA)
            {
                perror("malloc failed");
